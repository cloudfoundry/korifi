...
var _ = Describe("PackageHandler", func() {
	...
	Describe("the POST /v3/packages endpoint", func() {
		...
		When("the user is not allowed to create packages", func() {
			BeforeEach(func() {
				packageRepo.CreatePackageReturns(repositories.PackageRecord{}, repositories.NewForbiddenError(repositories.PackageResourceType, nil))
			})
	...
	Describe("the POST /v3/packages/upload endpoint", func() {
		...
		When("uploading the package is forbidden", func() {
			BeforeEach(func() {
				imageRepo.UploadSourceImageReturns("", repositories.NewForbiddenError(repositories.UploadSourceResourceType, nil))
			})
		...
		When("updating the package is forbidden", func() {
			BeforeEach(func() {
				packageRepo.UpdatePackageSourceReturns(repositories.PackageRecord{}, repositories.NewForbiddenError(repositories.PackageResourceType, nil))
			})
