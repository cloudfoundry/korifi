...
var _ = Describe("Parser", func() {
	var (
		...
		authInfoParser *authorization.InfoParser
		err            error
	)
	...
	JustBeforeEach(func() {
		authInfo, err = authInfoParser.Parse(authHeader)
	})
	...
	When("the Authorization header contains a Bearer token", func() {
		...
		It("extracts the token from the header", func() {
			...
			Expect(authInfo).To(Equal(authorization.Info{Token: "token"}))
		})
	...
	When("the Authorization header contains a ClientCert", func() {
		...
		It("extracts the cert and key data", func() {
			Expect(err).NotTo(HaveOccurred())
			Expect(authInfo).To(Equal(authorization.Info{CertData: []byte("foo")}))
		})
