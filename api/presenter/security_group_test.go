...
var _ = Describe("SecurityGroup", func() {
	...
	Describe("ForSecurityGroup", func() {
		JustBeforeEach(func() {
	...
	Describe("ForSecurityGroupStagingSpaces", func() {
		JustBeforeEach(func() {
			response := presenter.ForSecurityGroupRunningSpaces(record, *baseURL)
			var err error
			output, err = json.Marshal(response)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		It("returns the expected JSON", func() {
			Expect(output).To(MatchJSON(`{
				"data": [
  				...
  				"links": {
    			"self": {
      				"href": "https://api.example.org/v3/security_groups/security-group-guid/relationships/running_spaces"
    				}
  				}
			}`))
		})
	...
	Describe("ForSecurityGroupStagingSpaces", func() {
		JustBeforeEach(func() {
			response := presenter.ForSecurityGroupStagingSpaces(record, *baseURL)
			var err error
			output, err = json.Marshal(response)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		It("returns the expected JSON", func() {
			Expect(output).To(MatchJSON(`{
				"data": [
  				...
  				"links": {
    			"self": {
      				"href": "https://api.example.org/v3/security_groups/security-group-guid/relationships/staging_spaces"
    				}
  				}
			}`))
		})
