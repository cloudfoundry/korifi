	...
	Describe("GetRunnerInfo", func() {
		When("a handler requests the RunnerInfo for the configured runner", func() {
			When("the runner supports rolling deploy", func() {
				...
				JustBeforeEach(func() {
					runnerInfoRepo = NewRunnerInfoRepository(klient, runnerName, rootNamespace)
			...
			When("the runner does not support rolling deploy", func() {
				...
				JustBeforeEach(func() {
					noRollingDeployRunnerInfoRepo = NewRunnerInfoRepository(klient, noRollingDeployRunner, rootNamespace)
