	...
	Describe("ApplyToList", func() {
		...
		Describe("WithOrdering", func() {
			Describe("created_at", func() {
				BeforeEach(func() {
					option = repositories.WithOrdering("created_at")
				})
			...
			Describe("updated_at", func() {
				BeforeEach(func() {
					option = repositories.WithOrdering("updated_at")
				})
			...
			Describe("custom ordering", func() {
				BeforeEach(func() {
					option = repositories.WithOrdering("foo", "foo", "Foo")
				})
				...
				When("the orderBy is not in the mappings", func() {
					BeforeEach(func() {
						option = repositories.WithOrdering("bar", "foo", "Foo")
					})

					It("returns an error", func() {
						Expect(applyToListErr).To(MatchError(ContainSubstring("unsupported field for ordering: \"bar\"")))
					})
				})

				When("the mappings are invalid", func() {
					BeforeEach(func() {
						option = repositories.WithOrdering("foo", "key")
					})

					It("returns an error", func() {
						Expect(applyToListErr).To(MatchError(ContainSubstring("expected even number of key value pairs")))
					})
				})
			})
		})
