...
func main() {
	...
	configPath, found := os.LookupEnv("CONTOURROUTERCONFIG")
	if !found {
		panic("CONTOURROUTERCONFIG must be set")
	}

	contourRouterConfig, err := config.LoadFromPath(configPath)
	if err != nil {
		panic(err)
	}
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "3573e34c.cloudfoundry.org",
	...
	logger := ctrl.Log.WithName("contour-router").WithName("ContourRouter")

	if err = (controllers.NewCFRouteReconciler(
		mgr.GetClient(),
		mgr.GetScheme(),
		logger,
		contourRouterConfig,
