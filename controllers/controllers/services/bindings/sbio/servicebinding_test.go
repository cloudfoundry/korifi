...
var _ = Describe("SBIO", func() {
	Describe("ToSBServiceBinding", func() {
		...
		It("sets the status binding name to the sb binding spec name ", func() {
			Expect(sbioBinding.Spec.Name).To(Equal(cfServiceBinding.Status.Binding.Name))
		})
	...
	Describe("IsSBBindingReady", func() {
		...
		When("sbiobinding is outdated", func() {
			BeforeEach(func() {
				sbioBinding = &servicebindingv1beta1.ServiceBinding{
					Status: servicebindingv1beta1.ServiceBindingStatus{
