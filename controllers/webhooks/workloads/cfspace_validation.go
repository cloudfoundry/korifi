...
func (h cfSpaceHandler) handleUpdate(ctx context.Context, oldCFSpace, newCFSpace *workloadsv1alpha1.CFSpace) admission.Response {
	newAnchorName := newCFSpace.Spec.DisplayName
	if err := h.duplicateValidator.ValidateUpdate(ctx, h.logger, oldCFSpace.Namespace, oldCFSpace.Spec.DisplayName, newAnchorName); err != nil {
		if errors.Is(err, webhooks.ErrorDuplicateName) {
