...
var _ = Describe("CFAppValidatingWebhook", func() {
	var (
		...
		logger             logr.Logger
		duplicateValidator *workloads.DuplicateValidator
		err                error
		testName           string
		testNewName        string
	...
	BeforeEach(func() {
		...
		duplicateValidator = workloads.NewDuplicateValidator(nameRegistry)
		logger = logf.Log
	...
	Describe("ValidateCreate", func() {
		...
		When("the name is already registered", func() {
			...
			It("fails", func() {
				Expect(err).To(Equal(workloads.ErrorDuplicateName))
			})
		})
