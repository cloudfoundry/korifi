...
main() {
  ...
  tmux split-window -h -p 66 "GINKGO_NODES=2 /bin/bash -c \"make test-kpack-image-builder; bash --init-file <(echo 'history -s make test-kpack-image-builder')\""
  tmux split-window -h -p 50 "GINKGO_NODES=2 /bin/bash -c \"make test-stset-runner; bash --init-file <(echo 'history -s make test-stset-runner')\""
